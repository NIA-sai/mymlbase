...
uint load_data( string filename, Tensor< double > *&data, Tensor< double > *&label, uint batch_size = 250, uint data_size = 28 * 28, uint label_size = 1, bool has_header = true, uint claz_cnt = 10 )
{
	...
	{
		data[i] = raw_tensor( i * batch_size, ( i + 1 ) * batch_size, 1 )( label_size, -1, 1, 1 ).copy();
		label[i] = raw_tensor( i * batch_size, ( i + 1 ) * batch_size, 1 )( 0, label_size - 1, 1, 1 ).oneHot( claz_cnt );
	}
	return batch_cnt;
}
...
struct BP_MLP
{
	...
	double train( const Tensor< double > &x, const Tensor< double > &y )
	{
		...
		// cout << t4.tensor[0] << endl;
		// cout << t3.grad().tensor[0] << endl;
		...
		b1 -= ( b1h.grad().tensor * learn_rate )( 0 );
		cout << w2[0];
		w2 -= w2h.grad().tensor * learn_rate;
		cout << w2[0];
		// cout << b2[0];
		b2 -= ( b2h.grad().tensor * learn_rate )( 0 );
		// cout << b2[0];
...
int main()
{
	const uint batch_size = 10, turns = 10;
	...
	mlp.init( Tensor_Initializer::zeros< double >, Tensor_Initializer::zeros< double >, Tensor_Initializer::zeros< double >, Tensor_Initializer::zeros< double > );
	double loss;
	...
	cout << "test:" << mlp( train_data[0] );
	cout << "anser: " << train_label[0] << endl;
	delete[] train_data;
	delete[] train_label;
	// delete[] test_data;
	// delete[] test_label;
